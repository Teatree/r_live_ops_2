...
engine/pre_collection/  the PRE-COLLECTIONS variant of the engine (v4 + Daily + PBP + SimPerSegment,
              ...
              D19/D21 pack features; harnesses do NOT scan this folder
...
All five harnesses are GREEN as of 2026-08-05 — a FAIL means you broke something, not that a gate is stale. Gates must never bake workbook state into an assertion: snapshot the sheet, mutate, assert, restore (three gates had rotted into asserting that sheets were ABSENT which the workbook now ships; they were refreshed in D19).

`harness/_mockdata.json` is a dump of the live workbook's sheets (values + merges). `_dump_mockdata.py` also OVERLAYS any sheet listed in its `PENDING_IMPORT` map from the freshly built `display/*.xlsx`, so the harness tests the layout the engine expects even before you import the sheet (currently `PackConfig` and `SimOutput`; drop an entry once imported).
...
Workbook (13) state: `SP_v2`/`SP_lb_v2` carry **real edits** (halved Core-SPT panel, `Season Length 28`), the base `RM` sheet is DELETED (`RM_1st`/`RM_2nd` carry the ladders), and the base `Race` ladder pays Flash Race Coins 10 + SPT 50. **Season-Pass tier-30 cap (open flag):** `40-99` and `100+` exceed the 30-tier ladder on BOTH sides, so their Season Pass row cannot respond to an SPT change at all — SPT gates must therefore run on a segment with headroom (`SP_SEG = '10-19'`); `_mock_run` reports which segments are cap-masked.
