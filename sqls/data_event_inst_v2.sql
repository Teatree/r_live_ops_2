...
-- Grain: event x segment x payer  ·  Legacy reference output, not exported (v3 is canonical)
-- Window and selected arm use the shared inputs; dates are inclusive.
--
-- v2 adds Level Race using the same event_start/event_end + level_summary active_event_ids path
-- used by sibling daily leaderboard events. Current data_gains routes source_detail='race_event'
-- to 'Level Race'. The 2026-04-13 race_event reward step-change is before this window.
-- =============================================================================
WITH params AS (
    SELECT
...
ab_groups AS (
    ...
    WHERE app_id = 'abgbproduction_174525b3'
      ...
      AND snapshot_date = (
          SELECT MAX(event_date) FROM experiment_results.metrics
          WHERE app_id = 'abgbproduction_174525b3'
            AND rule_id = {{ gains_ab_rule_id_input }}
      )
...
active_player_days AS (
    ...
    FROM abgbproduction_174525b3_gdpr.player_daily d
    INNER JOIN ab_groups ab ON d.player_id = ab.player_id AND d.event_date >= ab.join_date
...
parts AS (
    SELECT i.event_id, dl.player_id, SUM(dl.tokens) AS tokens
    FROM (
...
master AS (
    SELECT aw.event_name, aw.segment, aw.seg_rank, aw.payer_flag, aw.event_id, aw.player_id,
        ...
        IF(aw.event_name = 'Level Race', COALESCE(en.token_balance, p.tokens), en.token_balance) AS token_balance,
        en.bots
    FROM aw
