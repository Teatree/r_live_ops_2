 ...
 *      EXCEPTION since D22: Night Sky reads NS on cal_curr / NS_v2 on cal_new, because the
 *      window sim anchors NS on measured and prices the redesign as E(NS_v2)/E(NS).
...
// 21, not 33 — the A/B window this workbook simulates on top of. Declared here as well as in
// EcoGainsSim_v4.gs (SIM_DAYS) and EcoGainsSim_Daily.gs (DAILY_DAYS) because Apps Script loads the
// files alphabetically and a load-time read of another file's var is undefined. The harness gates
// that all three match; change them together.
var PBP_DAYS = 21;
...
function pbpArgs_(calendar, day, segment, payer, mode, luck, seed, levels, startLevel){
  ...
  return { cal: cal, day: Math.max(1, Math.min(PBP_DAYS, Math.round(num(day)||1))),
...
function pbpSimulate_(a){
  ...
  // redesigned rewards on the current calendar, i.e. a ratio of 1 the window sim does not agree
  // with. (Same per-calendar treatment as c_saga/c_day/SP.)
