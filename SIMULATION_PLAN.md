...
| D24 | STOCHASTIC card sim: `SimulateCardCloud` (2026-09-01) | ✔ The card sim played ONE player once, which debugs mechanics and cannot answer design questions - a single seed cannot say whether 35 packs is typical or lucky (the suspected pack overshoot took 200 manual re-runs to settle as variance). New menu item **EcoGainsSim ▸ Simulate card cloud (all segments)** runs **N players (default 50, input cell) x 10 permutations** (5 real segments x 2 payer flags; **`A. 0` excluded by construction** - `data_seg_beh` has no row for it, so nothing can price its reach) and reports the DISTRIBUTION. **Refactor first:** `SimulatePackOpenings` was one 600-line function with all state in closures; split along SHEET ACCESS into `loadCardCatalog_` / `cardSeasonPre_` / `runOneCardSeason_` (pure - no SpreadsheetApp, no Math.random, seed fully determines it) with the old function as a thin wrapper. **Exactly one copy of the season rules**, so the single-player log and the cloud cannot describe different games - gated by `the two entry points share one season core`. Verified byte-identical through the refactor: SHA-256 over 4 segment/payer pairs x 2 seeds unchanged. **Cost:** the expensive calls (`packGrantPlan_`, `spPackTiers_` - each a calendar + config-sheet walk) are hoisted to once per permutation, and PackConfig + the catalog to once per sweep, so 500 players cost **10 engine walks, not 500** (3.9s in node for 50x10). The run logs elapsed time per permutation and stops cleanly with a message rather than dying mid-write at Apps Script's 6-minute limit. `playerSeed_` avalanches (seed, permutation, player) rather than `base+k` - mulberry32 seeds one apart give related first outputs, and 500 near-adjacent seeds is where that would surface as structure in the percentiles. **Output = 2 sheets** (`builders/_build_cardcloud.py`, one script two xlsx because they share every geometry constant): `Col_Cards_Cloud` - six CUMULATIVE series (Packs Opened, Cards Drawn, Unique Cards, Sets Completed, Album %, Star Balance) as p10/p25/p50/p75/p90/MEAN per day per permutation, plus a MEANS block putting all 10 permutations of one metric on one axis for cross-segment charts, plus the single Gemini chart prompt; `Col_Cards_Totals` - totals (mean + p10-p90), cadence per CALENDAR day (all 33, so permutations stay comparable), economy impact (the collection feature's OWN payout - SET + ALBUM REWARDS - in the config sheet's 21 `REWARD_COLUMNS`, split total / from sets / from albums), unlimited boosters in minutes behind a labelled minutes-per-unit INPUT the engine reads and never writes (blank → `-`, never a number nobody chose), and packs + cards per source. **Ranges are p10-p90 across players, NOT min-max** (user decision): with 50 players the true extremes are single outliers that move every run. `Unique Cards` counts `totalNew`, not `collectionSize`, and `albumPct` is `albums_done x 100 + progress` - both because the per-album counters RESET on album advance, so a progress curve built from them falls to zero exactly when a player does well; 133% is a real reading. Per-source rows show ZEROS deliberately - a zero is a finding (Kite at its D25 0.35 opt-in, a ladder with no pack authored), a missing row reads as a plumbing failure. Every block on both sheets is located by scanning column A for its BAR LABEL, exactly as `loadPackConfig_` does with PackConfig, so row numbers are not load-bearing; the only strings shared between builder and engine are those labels plus `CLOUD_BAND_STRIDE`/`CLOUD_SRC_ROWS` - all other text is written by the engine at run time. New harness `harness/_mock_cloud.js` (26 gates), reusing `_mock_cards.js`'s mock rather than keeping a second copy. |
| D24a | Three real bugs found while building D24 (2026-09-01) | ✔ All pre-existing, all invisible until the card harness could run again. **(1) `CHEST PURCHASING` was never read.** The sheet heading had been hand-renamed `CHEST PURCHASING (SIM CONTROLS)` while `loadPackConfig_` matched it exactly, so `buyMinStars`/`buyStartDay`/`buyEndProb` came back undefined and degraded to `minStars=Infinity, endProb=0`: **NO CHEST WAS EVER BOUGHT in a live run** - 'Stars Spent on Chests' was structurally 0, not merely zero - and with no label to bound it the STAR CHEST block ran past its own rows and swallowed the three purchasing parameters as fake chests ('Min Stars to Consider Buying' at 250 stars, reward pack 'no purchase below this star balance'). Nothing logged. Block labels now match exact-first then by prefix at a word boundary, so a trailing parenthetical is tolerated but `SET REWARDS` still cannot match `SET REWARDSX`; a prefix resolution and an unreadable chest panel are both logged. **(2) `spPackTiers_` ignored the `A. 0` guard.** `packGrantPlan_` refuses A. 0 (zero activity rates → empty plan) and `colRewardRow_` carries an explicit guard, but the Season Pass track is not instance-shaped - it pays every tier up to the one reached and has no reach term that could be zero - so A. 0 was handed 1 pack (NONPAYER) or 2 (PAYER) out of an otherwise empty season, exactly the tier coupling §3 says is not applied to it. **(3) dead `dailyPacksFor_` call**: assigned and never read, a leftover from the pre-2026-08-20 acquisition model, running `dailySeries_` over all 25 categories on every run purely to be discarded - the single most expensive thing the card sim did, and it would have run once per simulated player. Also removed `provRand`, a fourth RNG stream declared and never read. |
| D24b | Card harness un-blocked and de-rotted (2026-09-01) | ✔ `_mock_cards.js` had been **unrunnable for weeks**: `_dump_mockdata.py` globbed only `NEW_LIVEOPS_CALENDAR_ECO*`, so the card-collection workbooks could not be dumped at all, and the ECO workbook of record reverted `PackConfig` pre-D19 - `loadPackConfig_` threw 'PACK DEFINITIONS block is empty' before a single gate ran. Dumper now takes `--workbook` / `--out` (glob stays the default) and stamps a `_meta` record; `_mock_cards.js` takes `--data <collections|main|wb14|path>`, defaulting to the collections dump. `PENDING_IMPORT` emptied of `Col_Cards_Daily` - the workbook ships the current layout, so the overlay was replacing the real sheet, and the user's own added formulas, with a builder artefact. Then **5 rotted gates fixed by asserting the RULE, never by weakening**: the pity gates named 6-star as the tier that forces highest rarity (the ladder shifted down when the 6th tier was retired) → now round-trips whatever `PACK PITY CONFIG` says and reports it; 'no pack ladder authored yet → zero flow' asserted the sheet was unauthored → now mutation-gated (blank all 122 `*-star Dly` cells, assert zero, restore); attribution asserted the source set was EXACTLY the three sheets the fixture authors → now asserts authoring a pack makes that source appear and every attributed source is a real category; '1★ is the most-drawn rarity' ran on ONE seed where 1★ leads 2★ only 281:214 over ~220 draws (sd ~6 - a coin flip dressed as an assertion) AND is bent by three separate effects (set skew, rarity pity, and dry-streak pity, which forces UNOWNED cards and so steers away from the tier a player completes first) → now flattens all three, aggregates 6 seeds, and prints observed vs expected (1★ 412/435, 2★ 346/331, 3★ 243/221, 4★ 128/135, 5★ 64/79, Gold 72/63 - close to proportional, so the old failure was noise, not bias). NEW gate: the authored `ALBUM SET SKEW` must draw its favoured set harder than a neutral run (set 1: 210 → 265 cards) - the regression test the 2026-08-25 fix never got, when a 900 typed into that block did nothing at all. 70/9 → 82/0. |
| D25 | Kite Festival opt-in HARDCODED to 0.35 for the pack lane (2026-09-01) | ⚠ **AN ASSUMPTION, NOT A MEASUREMENT - 15-31x the observed rate. Revisit before quoting any Kite pack number.** Measured Kite opt-in in `data_event_inst` is **1-3%** (0.0096 at 0-9 NONPAYER .. 0.0327 at 100+ PAYER) because the Festival is a league you must JOIN. The pack lane prices packs as `E_v2 x participation x Sreach`, and the card sim gates each instance on `participation x reach`, so at the measured rate a pack typed onto every one of the 60 `Ki_v2` rank rows produced a Kite pack in only ~8% of runs (23/200 seeds at 20-39 PAYER) - indistinguishable from "Kite is not simulated". User decision (2026-09-01, verbatim: "let's just say that opt-in to Ki_v2 is 0.75", lowered the SAME DAY to 0.35 - "75% suddenly seemed kind of high" for an opt-in league): the REDESIGNED Kite is assumed to reach **0.35** opt-in. `PACK_PARTICIPATION = { 'Kite Festival': 0.35 }` in `EcoGainsSim_v4.gs`, applied via `packParticipation_`, which resolves: (1) a `Participation` label on the source's `_v2` config sheet, value in the cell to its right - AUTHORED, overrides the code with no edit; (2) the `PACK_PARTICIPATION` map; (3) the measured `participation_rate`; (4) 1.0 when there is no telemetry. Effect at 20-39 PAYER: Kite packs/season **0.0816 -> 1.1855**, runs granting >=1 Kite pack **23/200 -> 146/200** (at the interim 0.75 it was 2.5405 and 194/200). **SCOPE: the pack lane ONLY** - `rewardR_` is a v2/base ratio so participation cancels out of it, and Kite HC / boosters / SPT are untouched; pack flow for all 13 other sources is identical to the cent. Side benefit: authoring the number also defuses the rounded-export trap where a real 0.024 exports as `0.0`, which both readers treat as "no telemetry" and price at FULL participation (~40x too high) - the `_LIVEOPS_CALENDAR` export has exactly that for Kite, Level Race, Photoshoot, River Rush, Dream Pass and Season Pass Leaderboard. Gated by `_mock_cards.js` section 0, which PRINTS every active override on every run. |
| D26 | ENVELOPE season cutoff: packs stop at day 29 (2026-09-01) | ✔ The card-collection SEASON is shorter than the 33-day calendar window, so after `SEASON_LAST_DAY` no source hands out **envelopes** — the in-game name for the six `*-star Pack` resources. **NOTHING ELSE STOPS**: HC, SPT, SPTx2, boosters and Unlimited Lives are paid on all 33 days exactly as before, and no non-pack number moves by a single bit (gated). This is a pack-lane change, NOT a shorter simulation — the first draft of this decision read 'envelope' as a metaphor for the whole reward envelope and would have truncated the window, scaled carried totals and re-tiered the Season Pass; all of that was wrong and was discarded. Packs are simulated-side only (`data_gains` has no pack rows, anchor 0), so this is inherently a `cal_new` rule with no measured counterpart. **Two rules, both user decisions:** an instance ENTIRELY past the cutoff pays no envelopes — it leaves `packLane_`'s reach sum, so the window total itself shrinks (Night Sky -12.0%, Jigsaw -33.3%, Kite -20%, Target Day -20%, total envelopes -8.5% at 20-39 PAYER); an instance STRADDLING the cutoff pays IN FULL ("events cut in the middle we still give the full reward") with reach NOT clipped, but any envelope whose landing day falls past the cutoff settles on `SEASON_LAST_DAY` so the card sim never opens a pack with no album left to file it in (Hatchling Hideaway 27-30, Red's Challenge 29-30, Rainbow Maker #5 27-30 — all unchanged in value). **Season Pass is exempt** (`SEASON_EXEMPT_LANES`): its track is climbed during the season, so it pays its whole reached ladder even though the calendar draws a second pass instance at days 31-33. Choke points: `packLane_` filters the lane (one place, all three callers), `spPackTiers_` clamps its tier day, and in the daily view `packDayWeights_` gives the six pack columns their own day distribution while the other 13 keep the full 33-day one. The two branches that place days THEMSELVES had to be taught the rule separately — `nsDayTypeRows_` (Night Sky) and the Rainbow Maker per-instance branch — and the RM one leaked 0.41 packs onto day 30 until the gate caught it. `engine/pre_collection/` needs NO mirror: that copy has no pack lane at all (13 resources). Gated by 11 `_mock_cards` gates, all rule-based (each toggles `SEASON_CUTOFF` and compares, so none can rot); the load-bearing pair is 'no envelopes placed after SEASON_LAST_DAY' and its counterpart 'non-pack resources still paid on the days after the cutoff', which together pin the change to envelopes alone. Set `SEASON_CUTOFF = false` for the pre-D26 model bit for bit. |
| D27 | Leaderboard rank axis becomes a DISTRIBUTION (2026-09-02) | ✔ `lbE_` priced a leaderboard as the MEAN payout at exactly three integer ranks - `position_p25/p50/p75`, weight 1/3 each - and `packRungs_` reused the same three atoms. That is not a distribution, and on a top-heavy ladder it failed in both directions at once. **HARD ZERO** where no quantile lands in the paying band: Flash Race pays a pack at rank 1 only and `20-39` reads 2/3/4, so the model said that player wins none of **15** instances in 33 days - not rarely, never; likewise Red/Chuck/Bomb/Level Race at `0-9` (ranks 5/10/14 against a ladder paying 1-3) and Bomb at `10-19`. **OVER-GRANT** where one does: `40-99` Flash Race drew rank 1 with p=1/3 on EVERY instance (~5 outright wins a season), and integer quantiles COLLIDE at the top - `100+` Red reads [1,1,2], so rank 1 carried weight 2/3. User decision (2026-09-02): go with a real distribution, independent draw per instance, `avg_participants_per_instance` is the whole event population (NOT the bracket). The rank axis is now a piecewise-**linear** CDF through `(0,0), (p25,.25), (p50,.5), (p75,.75), (N,1)` with `P(rank=k) = F(k) - F(k-1)`; ties merge on the HIGHEST q, since a discrete `p_q` is the smallest rank with `F >= q`. `lbRankDist_` is SHARED by `lbE_` and `packRungs_`, so the gains model and the card sim reconcile by construction (gated per tier). **Why not a parametric fit:** `100+` Red has p25 = p50 = 1, i.e. at least half that segment's finishes are 1st, yet a lognormal least-squares through those anchors returns P(1) = 0.33 - it contradicts a fact the data states outright. The linear CDF honours every anchor exactly and assumes nothing else, and it is the CONSERVATIVE end of the feasible interval at the top (p25 = 1 only bounds P(1) to [0.25, 0.5); we take 0.25, where the old sampler took 0.333). **FLAGGED BIAS:** real rank distributions are unimodal, so where p25 is deep (`0-9`, p25 = 5) the true density at rank 1 is below the [1, p25] average and this reads a little high - 0.25/p25 = 0.05 per instance. Bounded by construction, and it replaces an exact 0. **N barely matters** - it only places the 25% above p75, past the end of every paying ladder - which deliberately defuses a live data conflict: `Race` declares `LBSize` 10 for Red/Chuck/Bomb while `data_event_inst` has p75 = 16-17 at `0-9`, so that panel is **stale** (Level Challenge 20 and Flash Race 7 agree with their p75). N is read per BLOCK via `blockLabel_` (`LBSize` on Race, `leagueGroupSize` on Ki) - `readSPLabel_` scans the whole sheet and collapsed all five Race panels onto Red's. **Effect** (33-day season, collections workbook): leaderboard packs **54.20 -> 50.49, -7%** - the structural zeros become small positives (Flash Race `20-39` 0 -> 0.38/season) while the top-segment over-grants come down (`40-99` Flash Race -25%, `100+` Red -22%); **Kite is invariant to the cent**, as it must be, because all 60 `Ki_v2` rows pay one pack and a flat ladder cannot care how the mass is spread. Beyond packs it **un-hides real config edits the carry rule was swallowing**: with `E_base = E_v2 = 0` at `0-9`, TaD_v2's halved LB coin ladder (200/100/50 -> 100/50/25) read R = 1 and now reads **R = 0.5**; Kite's 0.8x coin ladder previously reached only `40-99`. Whole-faucet HC **-0.20%**, LB HC -0.6%, LB SPT -1.1%. `LB_RANK_MODEL = 'quantiles'` restores the old sampler. Ported to `engine/pre_collection/` (non-pack feature). Gated in `_mock_cards.js` section 2b, all mutation fixtures. |
| D27a | A row that does not exist is not a rung (2026-09-02) | ✔ Found BY the D27 gates, not by reading. `LB_R_SPECS` declares Flash Race as rows 81..90, but the `Race` sheet ends at 87 and the ladder is 7 places (`LBSize` 7, `numberOfPositions` 7). `lbLadder_`'s ordinal fallback - which exists for `Ki_v2`'s formula-numbered position cells - read three MISSING rows as ranks 8, 9 and 10. Invisible under the three-atom sampler (p25/p50/p75 are all <= 7, so the phantoms were never looked up); under a rank DISTRIBUTION they take **12.5% of the mass and pay nothing for it**. Changes no shipped number today - Flash Race packs sit on rank 1, coins on 1-3, and the SPT leak cancels inside `R = E_v2/E_base` - but it understates any reward typed on ranks 4-7, which is exactly where a pack ladder would go next. `packBlockE_` carried the same defect and would have diluted its flat rank average. Both now skip a missing row, and use the ordinal fallback only for a row with actual content (blank, not zero - a rung that legitimately pays nothing is all zeros). The gate that caught it: a FLAT ladder must be invariant to the rank model; it read 3.2685 vs 2.8600 = x0.875 = F(7) with N stretched to 10. |
| D28 | Mighty Doors / Tower of Fortune SIMULATED, and tickets become resource 20 (2026-09-02) | ✔ ToF is the first source in the model that **SPENDS** a resource and the first whose payout is gated on a DECISION rather than on reach, so it is not `measured x R x D x T`: there is no measured anchor and none is possible for an event that has never run. Priced bottom-up as **`E[banked reward per run] x runs`**. **THE RUN** is a forward walk over `(stage, continues used, top-ups used)`: survive `H(s)` -> next stage; meet a pig -> continue with `p_c(k)` and RETRY THE SAME STAGE, else the run ends and **the whole pot is lost** (deck p7, CONFIRMED). Retrying is the honest reading of "continue" - you paid to undo the pig, not to skip ahead - and the state has to carry `k` because the price escalates and affordability depends on what is left of the wallet. **`p_c(k) = ContinueP_seg x A(balance_k / cost_{k+1})`, `A(r) = (r-1)/(r-1+h)`, 0 when `r < 1`** (hard floor: you cannot buy what you cannot afford). `h = TOF_AFFORD_H = 1`, user-approved after seeing that real wallets are small - `data_econ hc_balance_p50` runs 74..223 coins against a ladder starting at 50, so at h=4 the median player in every segment could afford only rung 1-2. The walk repeats at all four hc_balance percentiles and averages, the same way collections average a milestone ladder over the progress percentiles; p50 alone would erase the tail that reaches the deep rungs (p90 reaches rung 4-5). **PAYERS GET EXACTLY ONE TOP-UP** (user decision): without it the model says payers continue LESS, because measured payer balances are LOWER in every segment - they spend what they buy (100+ PAYER p50 = 74 vs NONPAYER 107). **THE LADDER LENGTH IS THE CONTINUE CAP** - a rung with no price cannot be sold - and `Max Continues per Run` lowers it. **TICKETS ARE EARNED, NOT RECHARGED** (deck p9, no storage cap): `ToF_Ticket` is appended as **resource 20** and priced by the same bottom-up lane the packs use (`BOTTOMUP_RES`), because it has the same measured anchor of 0. Runs come from a day walk - bank what the calendar paid that day, spend up to `Runs per Active Day`, carry the rest forward - so a ticket typed into a week-1 daily gift becomes a week-1 run. Tickets are **NOT** subject to the D26 envelope cutoff: ToF is its own always-on event. **THE SHEET** (`ToF`, was `MD`; `builders/_build_mightydoors.py` -> `display/ToF_v1.xlsx`) is authored inputs only - RUN CONFIG lost the five parameters nothing read, SEGMENT BEHAVIOUR gained a **MAX** ceiling row and a `Coin Balance override`, and the continue ladder is anchored to real wallet percentiles (cumulative 25/100/260/720 = under p25, p50, p75, p90) with growth capped at a rung **INDEX** so changing the multiplier moves the ceiling instead of needing a new number. Its four FORMULA sim blocks were **deleted**: once the engine had the real model they were a second, disagreeing one - they blend survive-and-revive into a single number, which cannot price an escalating ladder, cannot see the wallet and cannot express the top-up. Replaced by `ECOGAINS_TOF(payer, block, nonce)` spilling RUN / REWARD / GAINSPEND. GAINSPEND reports net in **coins, not a ratio** (the ratio hit +53,190% at stage 60) and blanks rows past a segment's cash-out (a flat negative across stages nobody plays reads as a loss that never happens). **RE-ENTRANCY, flagged:** ToF needs ticket income -> walks every source -> Season Pass -> `sptTotals_` sums SPT across ALL categories including ToF, which banks SPT. Broken by having the inner call return zeros; second order today, revisit if ToF becomes a large SPT faucet. **Effect** (20-39 NONPAYER, 16.44 tickets authored across HH/NS/J/Ph/Daily Gift/BB): 15.37 runs, P(a run pays) 0.17, 396.71 coins spent on continues over the window. 14 gates in `_mock_cards.js` section 7b, all mutation-fixtured. |
| D28a | Five reader bugs the ToF work exposed (2026-09-02) | ✔ All the same shape - a FIXED OFFSET where a LABEL was needed, or a resource count frozen at 19. **(1) `rewCols_` could not see resource #20.** Every `c1` in `LB_R_SPECS`/`COLL_R_SPECS`/`PACK_ONLY_SPECS` was authored against a 19-resource world and `ToF_Ticket` lands exactly one column past `c1` on all ten blocks, so a ticket typed by a designer produced **nothing, silently and indistinguishably from "none authored"**. The scan now self-extends past `c1` while headers keep naming a RESOURCE (a helper column never does), so resource #21 needs no edit. **(2) `packLane_` wrote only the six pack tiers**, so tickets - same zero anchor - could never flow at all. **(3) STAGES has three note rows** between its bar and its header, so `hdr = bar+1` landed on prose and the config came back with NO STAGE LADDER; CONTINUE COST gained two notes plus two input rows and `bar+2` read the notes as rungs; SEGMENT BEHAVIOUR's notes sit directly under the segment rows so "stop at the first empty column A" swallowed all five as segments named after their own prose. All three block headers are found by label now (`mdHeaderRow_`), and a segment row is one whose take-up cell holds a NUMBER. **(4) The rung-5+ price formula named its own cell** when chained off the row above - a circular reference Sheets reports even though the branch is never taken; it now grows from the last ANCHORED rung by a clamped exponent. **(5) `PENDING_IMPORT` was shadowing real sheets:** the workbook ships `Col_Cards_Cloud`/`Col_Cards_Totals` now, so the overlay was replacing them - and anything authored on them - with a builder artefact on every dump. Emptying it revealed neither was in `SHEETS`, so they had only ever reached the dump THROUGH the overlay, and `_mock_cloud` died on `data['Col_Cards_Totals'].values` before a gate could run. Also de-rotted three gates that froze old counts (`RESOURCES.length === 19`, `33 * 6` pack cells, chest `cost[0] === 1000`) and made `_mock_cloud`'s determinism gate immune to wall-clock - `SimulateCardCloud` stops early on a time projection, so the RESULT depended on machine load and the gate could fail for a reason that has nothing to do with determinism. NEW in `_mock_cards.js` section 0b: the EcoGainsSim header block must equal `RESOURCES` name for name and its source labels must equal `CATEGORY_ORDER` row for row - the pair that would have caught both this desync and the 2026-08-21 one unaided. |
