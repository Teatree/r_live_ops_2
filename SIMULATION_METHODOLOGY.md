...
4. **[R](#r-reward-ratio)** (`rewardR_` → `lbE_`): read `inst = ds.eventInst(name, seg, payer)` ← 📊
   `data_event_inst`. Build a **rank distribution** (`rankDist_`, D27): a piecewise-linear CDF through
   ...
   their position cell, ordinal fallback — but a row that does not EXIST is not a rung, D27a).
...
columns before declaring a row dead. Since D27 a deep-finishing segment gets a SMALL POSITIVE rather
...
- **rank_p25/50/75** — the segment's typical finishing ranks as
  ...
  every rank on the board carries some probability (D27).
